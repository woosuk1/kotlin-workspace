...
fun main(args: Array<String>) {
    ...
    /*
     * 처음 선언 시에 비어있는 상태로 생성되는 리스트는 값이 대입되지 않아 타입 정보를 유추할 수 없으므로 에러가 발생한다.
     * 리스트에 포함될 자료의 타입을 명시해야 한다.
    * */
...
//    println("mutableList default init: ${mutablelist.toString()}")  // [] toString
    ...
    println("mutableList remove: $mutableList")
    ...
    /* contains: 요소 포함 여부 */
    println("mutableList contain 달기: ${mutableList.contains("딸기")}")
    println("mutableList contain 포도: ${mutableList.contains("포도")}")

    /* size: 요소 개수 */
    println("mutableList size: ${mutableList.size}")

    /* 설명. listOf: 변경 불가능한 리스트(immutableList)를 선언하는 함수 */
