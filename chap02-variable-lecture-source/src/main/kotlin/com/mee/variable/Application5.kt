 ...
 *  확인하는 과정을 생략, 혹은 변수에 null값이 대입됐을 수도 있다는 상황을 가정하지 않고 코드를 작성해서
 *  NullPointerException이 발생하는 상황이 많다.
 *  코틀린은 변수의 타입을 기본적으로 null값의 대입이 허용되지 않는 타입으로 선언하게 되어 있고
...
//    s = null
    ...
    * */
    ...
    /* 1. let 함수
     *  let 함수는 변수의 값이 null이 아닌 경우에 실행할 코드 블럭을 작성할 수 있게 도와주는 함수
     *  it: value-paramter인 c가 null이 아닌 경우 c를 의미한다.
    * */
    c?.let{
    ...
    * */
    ...
    println("안전 호출 통한 문자열 길이 확인: $len")

    /* 3. elvis 연산자
     *
     * 연산자 왼쪽의 피연산자가 null이 아닐 경우 해당 값을 반환하고 null일 경우 오른쪽 피연산자를 반환한다.
    * */
    var one = null ?: 1 //왼쪽 피연산자 값이 null이므로 오른쪽 피연산자값(1)을 반환
    println(one)

    var two = 2 ?: 1 //왼쪽 피 연산자의 값이 null이 아니므로 왼쪽 피 연산자 값(2)를 반환
    println(two)
...
//    var wrong = wrongNumberString.toInt()
    var wrong = wrongNumberString.toIntOrNull()
